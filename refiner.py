...
# -*- coding: utf-8 -*-
"""
GT-FREE, PART-AWARE PROMPT REFINER (FAST)
...
def sample_feature_at_click(phi_hw_c: np.ndarray, x: float, y: float) -> np.ndarray:
    ...
    # v is not necessarily unit; normalize
...
def superpixel_stats(sp: np.ndarray,
    ...
    # adjacency & boundary strength
    boundary_strengths: Dict[Tuple[int,int], float] = {}
    # compute edges by scanning boundaries
    for y in range(H-1):
        for x in range(W-1):
            a = sp[y,x]; b = sp[y, x+1]
            c = sp[y,x]; d = sp[y+1, x]
...
def build_rw_weights(phi_sp: np.ndarray,
    ...
    """
    N, C = phi_sp.shape
    # neighbor list from boundary map
    ...
    # precompute median centroid distance & 75p boundary for scaling
    all_d = []
    all_b = []
    ...
    # self-tuning bandwidth sigma_i: kth neighbor df among neighbors
    sigma = np.zeros((N,), dtype=np.float32) + 1e-3
    # build per-node feature distances list
...
def random_walker_superpixels(W: sparse.csr_matrix,
    ...
    if len(unlab_idx) == 0:
        # All labeled; probabilities are one-hot
    ...
    P[unlab_idx] = P_u
    P[seeds_idx] = Ys
    # Normalize rows to sum to 1 & clamp
...
def build_local_regions(image_rgb: np.ndarray,
                        ...
                        min_link_dist: float,
                        margin: float,
                        progress: bool) -> Tuple[Dict[str, List[int]], Dict[str, np.ndarray], np.ndarray]:
    """
    Returns:
      groups: {"pos":[group_ids...], "neg":[group_ids...]}, each group id maps to indices of clicks
      regions: dict[group_name] -> boolean mask Ω_g (H,W)
      group_of_click: (N,) array with group name for each click
    """
    H, W = image_rgb.shape[:2]
    ...
    for ctx in it:
        ...
        Cc, hh, ww = emb.shape[1], emb.shape[2], emb.shape[3]
    ...
    if len(pos_idx)>0:
        ...
        # distance threshold: median pairwise distance * 0.6 (data-driven)
        if len(pos_idx) > 1:
    ...
    # helper: assign a superpixel to group label; if conflicts, prefer closest click in that sp
    def assign_seed_for_click(idx_click: int, gname: str):
        x, y = coords[idx_click]
        s = int(sp[int(round(y)), int(round(x))])
        if seeds[s] == -1:
            seeds[s] = len(group_names)  # will be overridden below if not yet added name
        # record click
    ...
    # 7) Define Ω_g via margin and (optionally) TTA-majority — here we used TTA-averaged embeddings so single pass is fine
    regions = {}
    Pmax_others = np.max(Ppix, axis=0, keepdims=True)  # (1,H,W)
    ...
    return {"pos": groups["pos"], "neg": groups["neg"], "names": group_names, "click_map": group_of_click}, regions, sp
...
def estimate_envelopes_local(grayL, ctxs, coords_all, labels_all, multimask, jitter_radius, rng,
                             ...
                             env_samples: int, eps: float, progress: bool):
    """
    For each group g: envelopes for U,R,E computed only on Ω_g.
    We reuse the same p_mean masks; we just mask to Ω_g when computing U,R,E.
    """
    H, W = grayL.shape[:2]
    n = len(coords_all)
    ...
    for gname, Omask in iterator:
        ...
        for _ in range(env_samples):
            ...
            # approximate TTA consistency locally by entropy only (pairwise IoU on Ω)
            U_loc = H_pred
            ...
            gray_local = cv2.cvtColor((p*255).astype(np.uint8), cv2.COLOR_GRAY2BGR)  # not used; keep E via global grayL:
...
def chebyshev_objective(grayL, ctxs, coords, labels, multimask, jitter_radius, rng,
    ...
    """
    Returns F (robust Chebyshev global + worst local), global breakdown br_g, and local breakdown dict {gname: {...}}
    """
    ...
    for gname, Omask in regions.items():
        # compute local terms using mask
        yx = Omask
        # reuse p
        p_local = p * yx
        m_local = (p_local>=0.5).astype(np.uint8)
        ...
        # normalize with local envelopes
...
def bce_local_by_group(grayL, ctxs, coords, labels, multimask, jitter_radius, rng,
                       group_click_indices: Dict[str, List[int]],
                       group_names: List[str],
                       eps: float) -> Dict[str, float]:
    """
    Compute BCE restricted to clicks of each group (using the mask predicted from current coords/labels).
    """
    C, _, _, _, _ = evaluate_terms_fast(grayL, ctxs, coords, labels, multimask, jitter_radius, rng)
    # For local BCE we simply reuse the same C but average only over clicks in group when evaluating
    # To avoid another SAM run, approximate local BCE by re-weighing: with uniform average this equals global C if groups are similar in size.
    # For fidelity, we could re-evaluate with only group clicks; to keep speed, we return global C as proxy (tight if eps_g scales similarly).
    # If you want exact per-group BCE, run SAM again with only group clicks; omitted for runtime.
    return {gname: C for gname in group_names}
...
def feasible_seed_by_bce(grayL, ctxs, coords_all, labels_all, multimask, jitter_radius, rng,
    ...
    for g in pos_groups:
        ...
        # pick 'needed' clicks that most reduce global BCE when added
        candidates = []
        for idx in g:
    ...
    # Now reduce global BCE to <= eps by adding the best remaining click each step (greedy)
...
def mc_sufficiency(grayL, ctxs, coords_all, labels_all, multimask, jitter_radius, rng,
    ...
    for j in prompt_iter:
        ...
        while len(deltas) < mc_samples and attempts < mc_samples * 50:
            attempts += 1
            # sample random subset S from others
...
def main():
    ...
    groups_info, regions, sp_map = build_local_regions(
        image_rgb=image,
        ctxs=ctxs,
        coords=coords_all,
        labels=labels_all,
